...
* *backfill*: Controls whether a result found in a later remote storage backend
  ...
  (the default), *strict* and *disabled*. With *strict*, a failed backfill is a
...
----
remote_storage = redis://cache.example.com oci://registry.example.com/team/ccache/shared @credential-helper=pass gha://build
----
