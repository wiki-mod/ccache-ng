...
until the server retry window expires; reads continue.
...
OCI storage supports reads, writes and manifest deletion. Deleting a manifest
does not guarantee that the registry will immediately garbage-collect its
blobs; that is controlled by the registry.
