...
`ACTIONS_RUNTIME_TOKEN` from its process environment. The backend is not
created, and no HTTP request is made, if either value is missing. Do not put a
token in `ccache.conf`, `CCACHE_REMOTE_STORAGE` or a workflow environment file.
...
* *@prefix*: Cache-key prefix. The URL path is used when it is not set.
...
* *@connect-timeout*: Timeout for establishing an HTTP connection.
* *@operation-timeout*: Timeout for one HTTP operation.
...
URL format: `+oci://REGISTRY/REPOSITORY[/ccache/PREFIX]+`
...
source may be configured. Direct token attributes are rejected.
