...
----
remote_storage = redis://cache.example.com backfill=best-effort oci://registry.example.com/team/ccache/shared @credential-helper=pass gha://build
----

The example contains no credential. Set *read-only* on a backend that must not
receive writes. Use `ccache --print-stats` for the machine-readable
`remote_storage_hit`, `remote_storage_write`, `remote_storage_error` and
`remote_storage_timeout` counters.
