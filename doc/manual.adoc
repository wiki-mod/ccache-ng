...
remote_storage =
  ...
  oci://registry.example.com/team/ccache/shared @credential-helper=pass
  gha://build
----
...
URL format: `+oci://REGISTRY[:PORT]/REPOSITORY[/ccache/PREFIX]+`

This built-in backend stores ccache entries as OCI manifests and blobs. It is
suitable for registries such as GHCR. HTTPS is used by default. The optional
`/ccache/PREFIX` part separates the repository path from a cache-key prefix.
Each entry uses the complete lower-case hexadecimal ccache digest, optionally
with that prefix. The first `/ccache/` in the URL has this special meaning, so
a repository path cannot itself contain that exact path segment.
...
Use a Docker credential helper as the normal credential source. ccache invokes
`docker-credential-NAME get` and keeps the returned credential in memory. A
private credential file and a Linux systemd credential are alternatives for
environments where a credential helper is not available. Exactly one credential
source may be configured. Direct token attributes are rejected. No credential is
needed for a registry and repository that allow anonymous access.
...
* `+oci://ghcr.io/OWNER/REPOSITORY/ccache/project @credential-helper=pass+`
* `+oci://registry.example.com/team/ccache/release @prefix=builds+`
